...
export class SpatialStructure {
	...
	async setUp(modelID) {
		this.cleanUp();
		try {
			const spatialRels = this.api.GetLineIDsWithType(modelID, WebIFC.IFCRELCONTAINEDINSPATIALSTRUCTURE);

			const allRooms = new Set();
			const rooms = this.api.GetLineIDsWithType(0, WebIFC.IFCSPACE);
			for (let i = 0; i < rooms.size(); i++) {
				allRooms.add(rooms.get(i));
			}

			// First add rooms (if any) to floors
			const aggregates = this.api.GetLineIDsWithType(0, WebIFC.IFCRELAGGREGATES);
